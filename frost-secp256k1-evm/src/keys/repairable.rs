...
pub type Delta = frost::keys::repairable::Delta<Secp256K1Keccak256>;

/// A sigma value which is the output of part 2 of RTS.
pub type Sigma = frost::keys::repairable::Sigma<Secp256K1Keccak256>;
...
pub fn repair_share_part2(deltas: &[Delta]) -> Sigma {
    frost::keys::repairable::repair_share_part2::<Secp256K1Keccak256>(deltas)
}
...
mod tests {
    ...
    fn check_repair_share_part1() {
        ...
        frost_core::tests::repairable::check_repair_share_part1::<Secp256K1Keccak256, _>(rng);
    }

    #[test]
    fn check_repair_share_part2() {
        frost_core::tests::repairable::check_repair_share_part2::<Secp256K1Keccak256>(
            &REPAIR_SHARE,
        );
    }
    ...
    fn check_repair_share_part3() {
        let rng = rand::rngs::OsRng;
        frost_core::tests::repairable::check_repair_share_part3::<Secp256K1Keccak256, _>(
            rng,
            &REPAIR_SHARE,
        );
    }
